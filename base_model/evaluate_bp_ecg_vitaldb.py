import torch
import numpy as np
from scipy import stats
import torch.nn.functional as F

def rmse(pred, target):
    return torch.sqrt(torch.mean((pred - target) ** 2)).item()

def mae(pred, target):
    return torch.mean(torch.abs(pred - target)).item()

def min_rmse(pred, target, max_shift=10):
    best = float('inf')
    for shift in range(-max_shift, max_shift + 1):
        shifted = torch.roll(target, shift, dims=-1)
        best = min(best, rmse(pred, shifted))
    return best

def min_mae(pred, target, max_shift=10):
    best = float('inf')
    for shift in range(-max_shift, max_shift + 1):
        shifted = torch.roll(target, shift, dims=-1)
        best = min(best, mae(pred, shifted))
    return best

def ks_test(pred, target):
    stat, p = stats.ks_2samp(
        pred.numpy().flatten(),
        target.numpy().flatten()
    )
    return stat, p

# ── Load ECG→ABP VitalDB outputs ─────────────────────────────────────────────
gt  = torch.load('input_batch_vitaldb_ecg.pt',  weights_only=False).float().cpu()
gen = torch.load('results_batch_vitaldb_ecg.pt', weights_only=False).float().cpu()

# ── Align shapes ──────────────────────────────────────────────────────────────
# gen: (N, 3, 1, 500)   [N samples, 3 diffusion draws, 1 channel, 500 points]
# gt : (N, 1, 500)
gen_mean = gen.mean(dim=1)          # (N, 1, 500) – average over diffusion samples
gt_ds    = gt                       # already (N, 1, 500)

gen_flat = gen_mean[:, 0, :]       # (N, 500) generated ABP
gt_flat  = gt_ds[:, 0, :]          # (N, 500) ground-truth ABP

# ── Print raw stats ───────────────────────────────────────────────────────────
print("Before rescaling:")
print(f"  GT  — mean: {gt_flat.mean():.3f}, std: {gt_flat.std():.3f}")
print(f"  Gen — mean: {gen_flat.mean():.3f}, std: {gen_flat.std():.3f}")

# ── Rescale generated to match GT distribution (z-score → GT stats) ──────────
gt_mean      = gt_flat.mean()
gt_std       = gt_flat.std()
gen_mean_val = gen_flat.mean()
gen_std_val  = gen_flat.std()

gen_rescaled = (gen_flat - gen_mean_val) / (gen_std_val + 1e-8) * gt_std + gt_mean

print("\nAfter rescaling gen to match GT distribution:")
print(f"  GT           — mean: {gt_flat.mean():.3f}, std: {gt_flat.std():.3f}")
print(f"  Gen rescaled — mean: {gen_rescaled.mean():.3f}, std: {gen_rescaled.std():.3f}")

# ── Evaluate both raw and rescaled ────────────────────────────────────────────
for label, gen_eval in [("Raw (no rescaling)", gen_flat), ("Rescaled", gen_rescaled)]:
    r      = rmse(gen_eval, gt_flat)
    m      = mae(gen_eval, gt_flat)
    mr     = min_rmse(gen_eval, gt_flat, max_shift=10)
    mm     = min_mae(gen_eval, gt_flat, max_shift=10)
    ks, p  = ks_test(gen_eval, gt_flat)

    print("\n" + "=" * 55)
    print(f"  [{label}] BP Generation — VitalDB ECG→ABP")
    print("=" * 55)
    print(f"  RMSE        : {r:.4f}")
    print(f"  Min-RMSE    : {mr:.4f}")
    print(f"  MAE         : {m:.4f}")
    print(f"  Min-MAE     : {mm:.4f}")
    print(f"  KS Statistic: {ks:.4f}  (p={p:.4e})")

# ── Best-of-3 oracle (rescaled) ───────────────────────────────────────────────
print("\n--- Best-of-3 oracle (rescaled) ---")
best_rmse_list = []
for i in range(gt_flat.shape[0]):
    gt_i   = gt_flat[i]               # (500,)
    cands  = gen[i, :, 0, :].cpu()    # (3, 500)
    # rescale each candidate to GT stats
    cands_r = (cands - cands.mean(dim=1, keepdim=True)) / \
              (cands.std(dim=1, keepdim=True) + 1e-8) * gt_std + gt_mean
    rmses = torch.sqrt(((cands_r - gt_i.unsqueeze(0)) ** 2).mean(dim=-1))
    best_rmse_list.append(rmses.min().item())
best_of_3_rmse = float(np.mean(best_rmse_list))
print(f"  Best-of-3 RMSE: {best_of_3_rmse:.4f}")

# ── mmHg conversion (use same ABP stats as training) ─────────────────────────
ABP_MEAN = 82.26
ABP_STD  = 22.55

# Use the rescaled metrics as your main reference
r_main = rmse(gen_rescaled, gt_flat)
m_main = mae(gen_rescaled, gt_flat)

print("\n[mmHg Results (Rescaled)]")
print(f"  RMSE : {r_main * ABP_STD:.2f} mmHg")
print(f"  MAE  : {m_main * ABP_STD:.2f} mmHg")
print(f"  Paper target (UniCardio-F): 5.79 mmHg RMSE")
