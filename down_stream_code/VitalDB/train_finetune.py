...
print(f"ABP stats: mean={ABP_MEAN:.2f} | std={ABP_STD:.2f}")
...
# [VitalDB] signal_impute = imputation_pattern(signal.copy(), extended = True)
# [VitalDB] signal_noisy = AddNoise(signal.copy(), SNR = 15)

# [VitalDB] signal already a tensor from load_vitaldb()
# [VitalDB] mask = torch.tensor(signal_impute[1], dtype = torch.float32)
...
# [VitalDB] signal_impute = torch.reshape(signal_impute, [signal_impute.shape[0], 3*signal_impute.shape[2]])
# [VitalDB] signal_noisy = torch.reshape(signal_noisy, [signal_noisy.shape[0], 3*signal_noisy.shape[2]])

# [VitalDB] null = torch.zeros([signal.shape[0], int(signal.shape[1]/3)])

# [VitalDB] signal = torch.concatenate([signal, null], dim = -1)
# [VitalDB] signal_impute = torch.concatenate([signal_impute, null], dim = -1)
# [VitalDB] signal_noisy = torch.concatenate([signal_noisy, null], dim = -1)


# Leave-subject-out split
# ── VitalDB: splits already pre-made, no GroupShuffleSplit needed ──
